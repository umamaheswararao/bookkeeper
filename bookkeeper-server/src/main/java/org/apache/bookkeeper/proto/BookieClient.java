...
public class BookieClient {
    ...
    public void addEntry(final InetSocketAddress addr, final long ledgerId, final byte[] masterKey, final long entryId,
            final ChannelBuffer toSend, final WriteCallback cb, final Object ctx, final boolean fencing, final boolean recovery) {
        ...
        client.connectIfNeededAndDoOp(new GenericCallback<Void>() {
            ...
            public void operationComplete(int rc, Void result) {
                ...
                client.addEntry(ledgerId, masterKey, entryId, toSend, cb, ctx, fencing, recovery);
            }
        });
    }

    public void readEntry(final InetSocketAddress addr, final long ledgerId, final long entryId,
                          final ReadEntryCallback cb, final Object ctx, final boolean fencing) {
        final PerChannelBookieClient client = lookupClient(addr);
        ...
        client.connectIfNeededAndDoOp(new GenericCallback<Void>() {
            ...
            public void operationComplete(int rc, Void result) {
                ...
                client.readEntry(ledgerId, entryId, cb, ctx, fencing);
            }
        });
    }
    ...
    public static void main(String[] args) throws NumberFormatException, IOException, InterruptedException {
        ...
        for (int i = 0; i < 100000; i++) {
            counter.inc();
            bc.addEntry(addr, ledger, new byte[0], i, ChannelBuffers.wrappedBuffer(hello), cb, counter, false, false);
        }
        counter.wait(0);
